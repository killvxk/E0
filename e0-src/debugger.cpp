...
/*
 * Modifications Copyright 2025 Jex Amro (Square Labs LLC)
 */
...
void Debugger::OnUnhandledException() {
  SAY("OnUnhandledException()");
}

void Debugger::OnInputpoint() {
  SAY("OnInputpoint()");
}

void Debugger::OnStoppoint() {
  SAY("OnStoppoint()");
}

void Debugger::OnHardwareBreakpoint() {
  SAY("OnHardwareBreakpoint()");
}

void Debugger::OnSingleStep() {
  SAY("OnSingleStep()");
}

bool Debugger::OnMemAccess() {
  SAY("OnMemAccess()");
  return true;
}

bool Debugger::OnWatchpoint() {
  SAY("OnWatchpoint()");
  return true;
}

void Debugger::LogSuspendCount() {
  SAY("LogSuspendCount()");
}
...
void Debugger::LoadModuleSymbols(void *base_address) {
  mach_header_64 mach_header;
  GetMachHeader(base_address, &mach_header);
  ...
  for (int i = 0; i < symtab_cmd->nsyms; ++i) {
    ...
    if ((curr_symbol.n_type & N_TYPE) == N_SECT) {
      ...
      /* disable demangled names

      //int status;
      //const char* demangled_sname = abi::__cxa_demangle(curr_sym_name,0,0,&status);
      //if (!demangled_sname) demangled_sname = curr_sym_name;

      //printf("\t- %s\n", demangled_sname);
      symbol_address = (void*)((uint64_t)base_address - text_cmd->vmaddr + curr_symbol.n_value);
      //symbols[(uint64_t)base_address][(uint64_t)symbol_address] = std::string(demangled_sname);
      symbols[(uint64_t)base_address][(uint64_t)symbol_address] = std::string(curr_sym_name);
      */
...
void Debugger::OnModuleLoaded(void *module, char *module_name, char *path) {
  if (trace_debug_events) {
  ...
  if (mode == 1) { /* dyld_image_removing */
    ...
    for (uint32_t i = 0; i < all_image_infos.infoArrayCount; ++i) {
      ...
      if (mode == 2) { /* dyld_notify_remove_all */
        ...
        OnModuleLoaded(mach_header_addr, base_name, path);
      }
    }
    ...
    m_dyld_process_info_for_each_image(
      ...
      ^(uint64_t mach_header_addr, const uuid_t uuid, const char *path) {
        if (attach_mode || IsDyld((void*)mach_header_addr)) {
          char *base_name = strrchr((char*)path, '/');
          base_name = (base_name) ? base_name + 1 : (char*)path;
          OnModuleLoaded((void*)mach_header_addr, (char*)base_name, (char*)path);
        }
      });
...
int Debugger::HandleDebuggerBreakpoint() {
  ...
  if (!mach_exception->code[1]) {
    int ret = BREAKPOINT_SINGLESTEP;
  #ifdef ARM64
  ...
  if (breakpoint->type & BREAKPOINT_HARDWARE) {
    OnHardwareBreakpoint();
  } else {
    RemoteWrite(breakpoint->address, &breakpoint->original_opcode, sizeof(breakpoint->original_opcode));
  }

  if (breakpoint->type & BREAKPOINT_INPUTPOINT) {
      OnInputpoint();
  }

  if (breakpoint->type & BREAKPOINT_STOPPOINT) {
      OnStoppoint();
  }
...
void Debugger::HandleExceptionInternal(MachException *raised_mach_exception) {
  //LOG("raised_mach_exception->thread_port: %x\n", raised_mach_exception->thread_port);
  ...
  /*
  SAY("0x%llx | exception_type: %d EXC_CODE[0]: %lx EXC_CODE[1]: %lx Thread:%lx\n", last_exception.ip, mach_exception->exception_type, mach_exception->code[0], mach_exception->code[1], mach_exception->thread_port);
  ...
  */
  ...
  if ((mach_exception->exception_type == EXC_BREAKPOINT) && (mach_exception->code[0] & 102)) {
    if (OnWatchpoint()) {
      //LOG("ARCH_PC: %llx\n", GetRegister(ARCH_PC));
      return;
    }
  ...
  if ((mach_exception->exception_type == EXC_BAD_ACCESS) && (mach_exception->code[0] & 2)) {
  ...
  if ((mach_exception->exception_type == EXC_BREAKPOINT) && (mach_exception->code[0] == 1)) {
    //SAY("mach_exception->exception_type == EXC_BREAKPOINT\n");
    int breakpoint_type = HandleDebuggerBreakpoint();
    //LOG("ARCH_PC: %llx\n", GetRegister(ARCH_PC));
    //SAY("breakpoint_type: %d @ 0x%llx | EXC_CODE[0]: %lx EXC_CODE[1]: %lx\n",breakpoint_type, last_exception.ip, mach_exception->code[0], mach_exception->code[1]);
    
    //if (breakpoint_type & BREAKPOINT_INPUTPOINT) {
    //  return;
    //}
    // if (breakpoint_type & BREAKPOINT_HARDWARE) {
    //   return;
    // }
...
DebuggerStatus Debugger::DebugLoop(uint32_t timeout) {
  ...
  uint64_t debug_loop_begin_time = GetCurTime();
  ...
  while (alive) {
    ...
    //LOG("DebugLoop Got Exception\n");
    //LogSuspendCount();
    uint64_t end_time = GetCurTime();

    uint64_t debug_loop_time = end_time - debug_loop_begin_time;
    //LOG("debug_loop_time_elapsed: %lld\n", debug_loop_time);

    //LOG("debug_loop_time: %lld | total_evtime: %lld\n", debug_loop_time, total_evtime);

    if ((total_evtime) && (total_evtime < debug_loop_time))
      debug_loop_time = debug_loop_time - total_evtime;

    if (debug_loop_time >= (uint64_t)timeout)
      timeout = 0;

    //uint64_t time_elapsed = end_time - begin_time;
    //LOG("time_elapsed: %lld timeout: %ld (uint64_t)timeout: %lld\n",time_elapsed, timeout, (uint64_t)timeout);
...
void Debugger::StartProcess(int argc, char **argv) {
  ...
  posix_spawn_file_actions_destroy(action_ptr); // Clean up file actions
  posix_spawnattr_destroy(&attr); // Clean up attributes
